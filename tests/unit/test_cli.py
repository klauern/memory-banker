...
class TestMemoryBankerCLI:
    ...
    async def test_update_command_success(self, mock_echo, temp_project_dir, mock_api_key):
        ...
        # Mock the agents and memory bank
        with patch.object(cli.memory_bank, 'exists', return_value=True) as mock_exists, \
            ...
            mock_analyze.assert_called_once_with(temp_project_dir)
            mock_update_files.assert_called_once_with({"projectbrief": "# Updated Brief"})
            
            # Verify output messages
            mock_echo.assert_any_call(f"🔄 Updating memory bank for project at: {temp_project_dir}")
    ...
    @pytest.mark.asyncio
    @patch('click.echo')
    async def test_update_command_no_existing_bank(self, mock_echo, temp_project_dir, mock_api_key):
        ...
        with patch.object(cli.memory_bank, 'exists', return_value=False):
            ...
            mock_echo.assert_any_call("❌ No existing memory bank found. Use 'init' command first.")
    
    @pytest.mark.asyncio
    @patch('click.echo')
    async def test_refresh_command_with_existing_bank(self, mock_echo, temp_project_dir, mock_api_key):
        ...
        with patch.object(cli.memory_bank, 'exists', return_value=True) as mock_exists, \
            ...
            # Verify output messages
            mock_echo.assert_any_call(f"🔄 Refreshing memory bank for project at: {temp_project_dir}")
            mock_echo.assert_any_call("🗑️  Removing existing memory bank...")
    
    @pytest.mark.asyncio
    @patch('click.echo')
    async def test_refresh_command_no_existing_bank(self, mock_echo, temp_project_dir, mock_api_key):
    ...
    async def test_update_propagates_exceptions(self, temp_project_dir, mock_api_key):
        ...
        with patch.object(cli.memory_bank, 'exists', return_value=True), \
             patch.object(cli.agents, 'analyze_project') as mock_analyze:
